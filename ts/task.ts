/// <reference path="../public/js/svg.js"/>
/// <reference path="../public/js/lib/lib.js"/>
/// <reference path="../public/js/lib/jquery.min.js"/>
/// <reference path="../public/js/lib/bootstrap.min.js"/>
class Task {
	_element: HTMLElement | JQuery<HTMLElement> | HTMLDivElement | JQuery<HTMLDivElement>;
	_taskinfo: { id: string; task: string; bucket: string; status: string; time: number; notes: string; description: string; };
	_id: string;
	_bucketel: any;
	_bucket: string;
	_task: string;
	...
	_bob: any;
	_taskel: any;
	constructor(argmap: { object?: any; selector?: any; }) {
		if (argmap.selector) { // If the element already exists and we need to use that then fill all the data
			...
			this._completeel = jQuery('<button/>', { id: 'complete', "class": 'closebutton' }).append($("#checkbox-temp").clone(true)); // Build and append the container and checkbox
